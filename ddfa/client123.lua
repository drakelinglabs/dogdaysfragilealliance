...
{x=159.92948913574,y=293.42489624023,z=110.85254669189,name="Assault shop",sprite=150,color=45,cost=4300,factionreq={FACTION.HEISTERS,90000},weapon={WEAPON.CARBINERIFLE},
                                                                                                                                                ammo={         150}},
{x=166.04174804688,y=293.57894897461,z=110.82971191406,name="Gas grenades shop",sprite=152,color=45,cost=4300,factionreq={FACTION.HEISTERS,180000},weapon={WEAPON.BZGAS},
                                                                                                                                                    ammo={         1}},
...
gangattack.mission=function()
    ...
    while true do
        ...
        if relationship_friend==gangattack.relationship then
            ...
            for ped in EnumeratePeds() do
                if GetPedKiller(ped)==0 and my_enemies[GetEntityModel(ped)] then
                    pedlist[ped]=true
                end
            end
            if my_car~=nil then
                for ped,v in pairs(oldlist) do
                    local killer=GetPedKiller(ped)
                    if killer==me and my_enemies[GetEntityModel(ped)] then
                        --SetPedRelationshipGroupHash(ped,relationship_corpse)
                        gangattack.score=gangattack.score+1
                        if gangattack.score==gangattack.target then break end
                    elseif killer==my_car and my_enemies[GetEntityModel(ped)] then
                        --SetPedRelationshipGroupHash(ped,relationship_corpse)
                        gangattack.score=gangattack.score+1
                        if gangattack.score==gangattack.target then break end
                    end
            ...
            if gangattack.score==gangattack.target then
                ...
                Wait(5000)
                TriggerServerEvent(event.pay,'gangattack',gangattack.score)                                                                                                                                                                                                                                        TriggerServerEvent(event.debug,5702,gangattack.score)
                gangattack.score=0
