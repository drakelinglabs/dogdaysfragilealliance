...
Citizen.CreateThread(function()
    ...
    -- end
    local always_first_person=true
    ...
    while true do
        ...
        if sniperrifles[weapon] then
            if always_first_person then
                crosshair_default=false
        ...
        elseif IsPedInAnyVehicle(ped, false) then
            ...
            if always_first_person or not (crosshair_default or IsPedOnAnyBike(ped) or IsThisModelAJetski(model)) then
        ...
        if always_first_person then
            ...
            DisableControlAction(0,0,false) --lock camera change
        elseif force_first_person then
...
-- Citizen.CreateThread(function()
    -- local settings={}
    -- for i=0,235 do
        -- settings[i]=GetProfileSetting(i)
    -- end
    -- while true do
        -- Wait(0)
        -- for i=0,235 do
            -- local new=GetProfileSetting(i)
            -- if settings[i]~=new then
                -- print("ProfileSetting "..i.." changed from "..settings[i].." to "..new)
                -- settings[i]=new
            -- end
        -- end
    -- end
