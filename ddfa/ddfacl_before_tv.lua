...
local player={
    ...
    showbusmenu=false,
    callingpolice=false,
    steam='steam_error',--text
    steam_high=0,--higher dword
    steam_low=0,--lower dword
}
...
decor.steam_high="steam_high"
decor.steam_low="steam_low"
decor.owner_id="veh_owner_id"
...
DecorRegister(decor.owner_id,3)
DecorRegister(decor.steam_high,3)
DecorRegister(decor.steam_low,3)
...
local garages={
    ...
    {x=-365.51022338867,y=6334.248046875,z=29.434747695923,angle=220.84080505371,radius=16}, --open garage in paleto
    {x=392.31408691406,y=430.83920288086,z=143.0068359375,angle=352.59094238281,radius=16}, --stilt right
    {x=353.50564575195,y=437.51190185547,z=146.34077453613,angle=296.95291137695,radius=16}, --stilt 
    {x=132.03175354004,y=567.87658691406,z=183.15789794922,angle=3.6926622390747,radius=16}, --stilt 
    {x=-189.54005432129,y=502.19567871094,z=134.02555847168,angle=23.3805809021,radius=16}, --stilt 
    {x=-555.03112792969,y=665.66827392578,z=144.52377319336,angle=337.38082885742,radius=16}, --stilt 
    {x=-684.97686767578,y=602.82952880859,z=143.18431091309,angle=50.64155960083,radius=16}, --stilt 
    {x=-753.73315429688,y=628.83166503906,z=142.17070007324,angle=300.96566772461,radius=16}, --stilt 
    {x=-862.76947021484,y=699.02545166016,z=148.61196899414,angle=340.13043212891,radius=16}, --stilt 
    {x=-1297.0166015625,y=457.17779541016,z=96.979309082031,angle=342.6630859375,radius=16}, --stilt 
    {x=-62.089931488037,y=6444.2392578125,z=31.077264785767,angle=45.071678161621,radius=16}, --paletoopensmall
}
...
RegisterNetEvent(event.my_steam)
AddEventHandler(event.my_steam,function(steam)
    local longlong=tonumber(steam,16)
    player.steam=steam
    player.steam_low=longlong&0xFFFFFFFF
    player.steam_high=(longlong>>32)&0xFFFFFFFF
    print("my steam is: "..string.format("%X",player.steam_high)..string.format("%X",player.steam_low))
end)
...
Wait(5000)
    ...
    while true do
        ...
        if IsPedInAnyVehicle(ped, false) and not player.is_cop and not player.civilian then
...
AddEventHandler(event.startheist, function(id,heist)
    ...
    local wantcolors={[0]=4,[1]=2,[2]=2,[3]=46,[4]=51,[5]=1,[6]=40}
...
for k,v in pairs(SKINS.REPORTERS) do
    civplayermodels[v]=true
end
for k,v in pairs(SKINS.COASTGUARD) do --workaround
    civplayermodels[v]=true
end
...
Citizen.CreateThread(function()
Wait(5000)
 ExpandWorldLimits(world.maxx+66.6,world.maxy+66.6,9999.9)
 ...
 while true do
  ...
  if pos.x>world.maxx-600.0 or pos.x<world.minx+600.0 or pos.y>world.maxy-600.0 or pos.y<world.miny+600.0 then
   if GetPlayerWantedLevel(playerid)>0 then
    ...
    if GetFakeWantedLevel()~=0 then SetFakeWantedLevel(0) end
    player.wanted=0
  ...
  elseif player.wanted==6 then
   ...
   elseif GetPlayerWantedLevel(playerid)~=5 then
    ...
    SetPlayerWantedLevelNow(playerid, false)
    if GetFakeWantedLevel()~=6 then SetFakeWantedLevel(6) end
...
Wait(5000)
    while true do 
        ...
        elseif player.wanted == 5 then
            SetNotificationTextEntry("STRING");
            AddTextComponentString("You have 5 wanted level! Press ~g~H ~s~to surrender. You will save ~g~10% ~s~of your money.")
            DrawNotification(false, false);
        elseif player.wanted == 6 then
            SetNotificationTextEntry("STRING");
            AddTextComponentString("~r~You have 6 wanted level! Leave San-Andreas state to lose it.")
            DrawNotification(false, false);
            SetNotificationTextEntry("STRING");
            AddTextComponentString("You have 6 wanted level! Press ~g~H ~s~to surrender. You will save ~g~5% ~s~of your money.")
            DrawNotification(false, false);
        end
        Citizen.Wait(5000)
        if player.wanted > 0 then
...
AddEventHandler(event.wanted,function(level)
    ...
    if GetPlayerWantedLevel(playerid)<level then
        ...
        SetPlayerWantedLevelNow(playerid,false)
        if level==6 then player.wanted=6 end
    end
end)
...
Citizen.CreateThread(function()
    ...
    local prop_cs_server_drive=GetHashKey("prop_cs_server_drive")
    ...
    local boxes={}
    boxes[hei_p_attache_case_shut]={c=8000,h=0.11}
    local boxes_pile={
    ...
    local chemicals={}
    chemicals[v_ind_cs_chemcan]={c=4000,h=0.38}
    ...
    local heroin={}
    heroin[hei_prop_hei_drug_case]={c=20000,h=2.5}
    heroin[hei_prop_hei_drug_pack_01b]={c=4000,h=0.14}
    heroin[hei_prop_hei_drug_pack_02]={c=2000,h=2.5}
    heroin[prop_drug_package]={c=20000,h=0.27}
    local heroin_pile={
    ...
    local coke={}
    coke[bkr_prop_coke_cutblock_01]={c=60000,h=0.05}
    local coke_pile={
    ...
    local gold={}
    gold[prop_gold_bar]={c=80000,h=0.06}
    local gold_pile={
    prop_gold_bar,
    prop_gold_bar,
    prop_gold_bar,
    ...
    local documents={}
    documents[prop_cs_documents_01]={c=2000,h=.01}
    documents[prop_cd_folder_pile4]={c=4000,h=.11}
    documents[v_res_paperfolders]={c=8000,h=.22}
    documents[prop_laptop_02_closed]={c=100000,h=.04,y=.12}
    ...
    local hdd={}
    hdd[prop_cs_server_drive]={c=10000,h=.03,y=.23}
    local hdd_pile={
...
--=GetHashKey("")
    ...
    while true do
                ...
                if square<2500.0 then
                    ...
                    if not player.is_dead then
                        ...
                        elseif key<0 and square<100.0 and not (player.is_cop or player.civilian) then
                            if v.wanted==6 and player.wanted~=v.wanted then
                                player.wanted=v.wanted
                                TriggerServerEvent(event.wanted,6)
                            else
                                local playerid=PlayerId()
                    ...
                    if key<0 then --heist
                        ...
                        elseif v.sprite==521 then -- laptop
                            universal_pile(v,key,hdd,hdd_pile)
                        elseif v.sprite==525 then -- hdd
                            simple_pile(v,key,documents,documents_pile,.46,0.5)
                        end
                    elseif money_bags[key]==nil then
                        local model
        ...
        if not player.civilian and closest~=nil and not IsPedInAnyVehicle(PlayerPedId()) then
            ...
            if v then
                ...
                elseif v.wanted==6 and player.wanted~=v.wanted then
                    player.wanted=v.wanted
                    TriggerServerEvent(event.wanted,v.wanted)
                else
                    local playerid=PlayerId()
                    local current_wanted=GetPlayerWantedLevel(playerid)
                    if v.wanted>current_wanted then
...
local function createmycar(hash,x,y,z,angle)
    ...
    SetBlipScale(carblip, 0.6)
    SetBlipColour(carblip, 3)
    DecorSetInt(mycar.veh,decor.steam_low,player.steam_low)
    DecorSetInt(mycar.veh,decor.steam_high,player.steam_high)
    DecorSetInt(mycar.veh,decor.owner_id,GetPlayerServerId(PlayerId()))
...
-- {x=-576.06091308594,y=291.23370361328,z=79.176666259766,name="SMG shop",sprite=150,color=45,cost=4500,weapon={WEAPON.SMG},
                                                                                                           -- ammo={         150}},
{x=517.27264404297,y=4835.9399414063,z=-66.1867218018,name="Flashlight",cost=0,weapon={WEAPON.FLASHLIGHT},
                                                                                 ammo={               1}},
{x=511.51156616211,y=4886.1596679688,z=-62.5920753479,name="Flashlight",cost=0,weapon={WEAPON.FLASHLIGHT},
                                                                                 ammo={               1}},
}
lspdheavy.weaponshops={
{x=452.37139892578,y=-980.0517578125,z=30.689596176147,fbi=true,name="Free police weapons",sprite=156,color=3,cost=0,weapon={WEAPON.COMBATPISTOL,WEAPON.PUMPSHOTGUN,WEAPON.SMG,WEAPON.NIGHTSTICK,WEAPON.STUNGUN},
                                                                                                                       ammo={          36,                  8,        30,               1,            1}},
}
...
faction.coastguard.weaponshops={
{x=-1178.548828125,y=-1781.1015625,z=4.1823129653931,fbi=true,name="Coast guard weapons",sprite=156,color=3,cost=0,weapon={WEAPON.PISTOL,WEAPON.SMG,WEAPON.FLAREGUN,},
                                                                                                                     ammo={           36,        30,              5,}},
}
lspd.weaponshops={
...
Wait(5000)
    ...
    makeblips(faction.sspd.weaponshops)
    ...
    hideblips(faction.sspd.weaponshops)
...
--{x=-1148.3089599609,y=-1518.8985595703,z=10.643747329712,name="Medkit",color=14,sprite=153,cost=1000}, -- floyd
--{x=-804.15350341797,y=169.9162902832,z=76.740180969238,name="Medkit",color=14,sprite=153,cost=1000}, -- michael
--{x=1274.5648193359,y=-1717.8825683594,z=54.771488189697,name="Medkit",color=14,sprite=153,cost=1000}, -- lester
--{x=-108.96404266357,y=-12.111380577087,z=70.519645690918,name="Medkit",color=14,sprite=153,cost=1000}, -- janitor
...
{x=1985.2779541016,y=3048.7067871094,z=47.215042114258,name="Drink",armor=50,color=23,sprite=93,cost=400}, -- yellow jack
{x=-560.26623535156,y=285.49325561523,z=82.176300048828,name="Drink",armor=50,color=23,sprite=93,cost=500}, -- tequilala
{x=135.64688110352,y=-1464.9498291016,z=29.357027053833,name="Food",color=23,sprite=52,cost=100}, -- lucky plucker
{x=128.2878112793,y=-1285.4421386719,z=29.279960632324,name="Drink",armor=50,color=23,sprite=93,cost=600}, -- strip club
{x=-1393.1754150391,y=-606.58898925781,z=30.319490432739,name="Drink",armor=50,color=23,sprite=93,cost=600}, -- bahama
}
 faction.lsfd.medics={
...
Wait(5000)
    ...
    makeblips(faction.lsfd.medics)
    makeblips(lspd.medics)
    makeblips(faction.sspd.medics)
    ...
    hideblips(faction.lsfd.medics)
    hideblips(faction.fbi.medics)
    hideblips(lspd.medics)
    hideblips(faction.sspd.medics)
                ...
                if (square<4) and (not player.is_dead) then
                    ...
                    elseif IsControlPressed(0, 86) then
                        if player.money>=v.cost then
                            ...
                            local goodheal=(v.armor~=nil and GetPedArmour(ped)<v.armor)
                            if GetEntityHealth(ped)<maxhealth or goodheal then
...
fasttravel={x=-273.46496582031,y=-953.27453613281,z=91.108337402344},
...
bomjatnya_colortv={cost=10000,welcome="buy ~b~cheap apartment~s~",firsttimecheck=true,
entrance={x=371.8544921875,y=-1791.1241455078,z=29.095457077026},sprite=40,
zone={x=151.36083984375,y=-1007.8784790039,z=-98.999984741211,r=50,h=5},
interiorid=149505,
weaponshop={x=154.81071472168,y=-1005.8638305664,z=-98.999961853027},
stash={x=151.80364990234,y=-1000.7803955078,z=-98.999961853027},
...
RegisterNUICallback('shop_buy_weapon', function(data, cb)
    ...
    if player.money~=nil and player.money>=data.cost then
        ...
        elseif data.weapon=="parachute" then
            GiveWeaponToPed(PlayerPedId(),WEAPON.PARACHUTE,1,false,true) --parachute
        else
            if (data.count) then
                        ...
                        then
                            ...
                            then
                                ...
                                if player.wanted>4 then
                                    SetNotificationTextEntry("STRING")
                                    AddTextComponentString("You can't use this with "..player.wanted.." stars.")
                                    DrawNotification(false, false)
                                else
                                    disable_weapons_in_apartments()
...
faction.news.skinshops={
  {x=-598.73706054688,y=-931.91882324219,z=23.864175796509,sprite=73,color=3,name="Change model",models={744705981}},
  {x=-598.728515625,y=-927.89794921875,z=23.862899780273,sprite=73,color=3,name="Change model",models={744705981}},
}
...
faction.criminal.skinshops={
    ...
    {x=1201.8514404297,y=2714.4287109375,z=38.222587585449,sprite=73,color=2,name="Binco shop",models=clothesskinsshops.binco}, -- GRAND SENORA
    
    {x=-1100.4555664063,y=2717.1372070313,z=19.107824325562,sprite=73,color=2,name="Binco shop",models=clothesskinsshops.binco}, -- zancudo
    ...
    {x=-277.68383789063,y=-936.60827636719,z=92.510887145996,sprite=73,color=4,name="Suits",models=clothesskinsshops.ponsonbys}, -- ALTA
    ...
    {x=105.31523132324,y=-1303.1232910156,z=28.768798828125,name="secret",models={GetHashKey("s_f_y_hooker_01"),GetHashKey("s_f_y_hooker_02"),GetHashKey("s_f_y_hooker_03"),1381498905,1846523796,1544875514,1446741360,GetHashKey("a_f_y_genhot_01"),GetHashKey("a_f_y_hiker_01")}},--vanilla unicorn
    ...
    {x=512.94775390625,y=4833.0244140625,z=-66.186637878418,name="secret",models={-265970301,-933295480}},--captain
...
Wait(5000)
    ...
    makeblips(faction.sspd.skinshops)
    ...
    hideblips(faction.sspd.skinshops)
...
NAVY={321739290,-823509173,-1281684762,782665360,970385471,-50547061,-42959138,276773164,74062924,1210229692},
MILITARY={321739290,-823509173,1210229692},
...
SAHP={-34623805,-1627000575,-114291515,1034187331},
 ...
 faction.criminal.carshops={
    ...
    {x=-478.01977539063,y=-614.94030761719,z=31.1744556427,sprite=326,name="Sport car shop",cost=15000,cars={-1622444098,1123216662,767087018,-1041692462,1274868363,736902334,2072687711,-1045541610,108773431,-566387422,-1995326987,-1089039904,499169875,-1297672541,544021352,-1372848492,482197771,-142942670,-1461482751,-777172681,-377465520,-1934452204,1737773231,1032823388,719660200,-746882698,-1757836725,1886268224,384071873}},
    {x=-1792.1674804688,y=458.46810913086,z=128.30819702148,sprite=326,name="Classic sport car shop",cost=30000,cars={-982130927,-1566741232,-1405937764,1887331236,941800958,223240013,1011753235,784565758,1051415893,-1660945322,-433375717,1545842587,-2098947590,1504306544}},
    {x=-64.245101928711,y=886.47015380859,z=235.82223510742,sprite=523,name="Super car shop",cost=100000,cars={-1216765807,-1696146015,-1311154784,-1291952903,1426219628,1234311532,418536135,-1232836011,1034187331,1093792632,1987142870,-1758137366,-1829802492,2123327359,234062309,819197656,1663218586,272929391,408192225,2067820283,338562499,1939284556,-1403128555,-2048333973,-482719877,917809321}},
    ...
    {x=2380.8640136719,y=3039.5324707031,z=47.689258575439,angle=0.3908,sprite=528,color=1,name="Special modified truck",cost=30000,cars={-1649536104}},
    ...
    {x=189.34603881836,y=282.08395385742,z=141.4781036377,angle=338.59439086914,name="Havok",cost=7000,sprite=574,factionreq={FACTION.HEISTERS,2000000},cars={-1984275979}},
    ...
    {x=1596.0075683594,y=-1651.6403808594,z=88.171897888184,angle=264.71340942383,sprite=326,color=24,name="Racing car shop",cost=17500,cars={-295689028,196747873,237764926,-1106353882,-631760477,-591651781,-915704871,349315417,-401643538}},
...
faction.news.carshops={
    {x=-285.87228393555,y=-618.1611328125,z=50.054058074951,angle=270.05,fbi=true,sprite=43,color=3,name="Helicopter",cost=0,cars={744705981}},
    {x=-592.55029296875,y=-921.41387939453,z=36.555065155029,angle=89.99,fbi=true,sprite=43,color=3,name="Helicopter",cost=0,cars={744705981}},
}
faction.lsfd.carshops={
...
faction.coastguard.carshops={
    {x=-1240.7269287109,y=-1864.2874755859,z=1.4682104587555,angle=134.1398,fbi=true,sprite=404,color=3,name="Police boat",cost=0,cars={-488123221}},
}
lspd.carshops={
...
navy.carshops={
    ...
    {x=-2192.1254882813,y=3265.7802734375,z=32.432655334473,angle=60.793060302734,fbi=true,sprite=576,color=26,name="Hunter",cost=0,cars={-42959138}},
    {x=-2342.302734375,y=3250.3903808594,z=32.447078704834,angle=329.43154907227,fbi=true,sprite=56,color=52,name="Squaddie",cost=0,cars={1210229692}},
    {x=-2347.0539550781,y=3242.3459472656,z=32.447418212891,angle=329.40353393555,fbi=true,sprite=56,color=52,name="Armed Squaddie",cost=0,cars={74062924}},
}
military.carshops={
 ...
 lost.carshops={
    {x=985.50256347656,y=-137.88551330566,z=72.8,angle=57.032344818115,rent=true,sprite=348,color=62,name="Bike",cost=500,cars={390201602,-1404136503,2006142190,741090084,301427732,-159126838,-1606187161,1873600305,-618617997,-1009268949,-570033273}},
    {x=971.02526855469,y=-114.53890228271,z=74.1,angle=223.27487182617,rent=true,sprite=326,color=62,name="Van",cost=500,cars={-1745203402},factionreq={FACTION.LOST,1000000}}
}
 merc.carshops={
    ...
    {x=503.46008300781,y=-3357.6987304688,z=5.8398675918579,angle=359.92239379883,name="Savage",rent=true,cost=25000,sprite=576,factionreq={FACTION.MERCS,12000000},cars={-82626025}},
    {x=503.36743164063,y=-3309.9672851563,z=5.8400278091431,angle=181.9877356290817,name="Akula",rent=true,cost=45000,sprite=576,factionreq={FACTION.MERCS,17000000},cars={1181327175}},
    {x=501.48458862305,y=-3179.8447265625,z=5.8389925956726,angle=0.40175178647041,name="Chernobog",rent=true,cost=10000,sprite=603,factionreq={FACTION.MERCS,1000000},cars={-692292317}},
...
local repair_wheels,repair_windows,repair_engine,repair_body,repair_paint,repair_nopaint,repair_full=1,2,4,8,16,15,31;
 faction.criminal.repairshops={
    ...
    {x=873.72680664063,y=-2197.7424316406,z=30.10597038269,sprite=72,color=40,color1=0,color2=0,name="Car paint (black)",cost=300,flags=repair_paint},
...
Wait(5000)
    ...
    makeblips(faction.lsfd.repairshops)
    ...
    hideblips(faction.lsfd.repairshops)
...
local function scuba_elite()
 ...
 faction.criminal.special={
    ...
    {x=513.0,y=4881.7,z=-62.5867,name="Scuba gear",cost=0,check=havescubagear,equip=scuba_criminal},--submarine
}
 faction.fbi.special={
    ...
    {x=513.0,y=4881.7,z=-62.5867,name="Scuba gear",cost=0,check=havescubagear,equip=scuba_fbi},--submarine
}
 merc.special={
    ...
    {x=513.0,y=4881.7,z=-62.5867,name="Scuba gear",cost=0,check=havescubagear,equip=scuba_merc},--submarine
}
 heisters.special={
    ...
    {x=513.0,y=4881.7,z=-62.5867,name="Scuba gear",cost=0,check=havescubagear,equip=scuba_heister},--submarine
}
 elite.special={
    ...
    {x=513.0,y=4881.7,z=-62.5867,name="Scuba gear",cost=0,check=havescubagear,equip=scuba_elite},--submarine
}
                ...
                if (square<v.radius) and (not player.is_dead) then
                    if IsControlPressed(0, 86) then
                        if v.car and not IsPedSittingInAnyVehicle(PlayerPedId()) then
                            ...
                            local c=v.car
                            local angle=v.angle
                            if angle==nil then angle=GetEntityHeading(PlayerPedId()) end
                            local veh=loadcar(v.x,v.y,v.z,angle,c)
                            v.car=nil
                            v.name="Garage"
                            --TaskEnterVehicle(PlayerPedId(),veh,1,-1,2.0,16,0)
                            SetPedIntoVehicle(PlayerPedId(),veh,-1)
...
for k,v in pairs(SKINS.COASTGUARD) do
allies_government[v]="coast guard"
end
for k,v in pairs(SKINS.REPORTERS) do
allies_government[v]="reporter"
end
...
VEHWHITELIST.REPORTERS={}
for k,v in pairs(VEHICLES.REPORTERS) do
    VEHWHITELIST.REPORTERS[v]=true
end
VEHWHITELIST.MEDICS={}
...
VEHWHITELIST.COASTGUARD={}
for k,v in pairs(VEHICLES.COASTGUARD) do
    VEHWHITELIST.COASTGUARD[v]=true
end
...
switch_to.coastguard=function()
    hide_phone_now=true
    Wait(30)
    TriggerServerEvent(event.gang,27)
    ...
    weaponshops=faction.coastguard.weaponshops
    carshops=faction.coastguard.carshops
    ...
    vehiclewhitelist=VEHWHITELIST.COASTGUARD
    basecoords={x=-1183.4830322266,y=-1773.9500732422,z=4.1827969551086,angle=18.77464294434}
    show_new_blips()
    hideblips(garages)
    ...
    player.special_ability=special_abilities.coastguard
    player.special_ability_amount=0
    player.special_ability_timestamp=nil
    SetPedAsCop(PlayerPedId(), true)
end

switch_to.reporters=function()
    hide_phone_now=true
    Wait(30)
    TriggerServerEvent(event.gang,26)
    relationship_friend=GetHashKey("CIVMALE")
    SetPedRelationshipGroupHash(PlayerPedId(),relationship_friend)
    hide_old_blips()
    hideblips(join_faction)
    medics=faction.criminal.medics
    skinshops=faction.news.skinshops
    weaponshops={}--lspdheavy.weaponshops
    carshops=faction.news.carshops
    repairshops=faction.criminal.repairshops
    mercenaries={}--lspd.mercenaries
    armorshops=faction.criminal.armorshops
    ...
    weapon_upgrade_shops={}--lspd.weapon_upgrade_shops
    special={}--lspd.special
    vehiclewhitelist=VEHWHITELIST.REPORTERS
    basecoords={x=-598.76989746094,y=-929.91925048828,z=23.863523483276,angle=90.001}
    show_new_blips()
    hideblips(garages)
    garages_enabled=false
    ...
    dont_kill_those=allies_government
    SetMaxWantedLevel(5) --0
    player.special_ability=special_abilities.reporter
    player.special_ability_amount=0
    player.special_ability_timestamp=nil
    --SetPedAsCop(PlayerPedId(), true)
end

switch_to.firefighters=function()
    hide_phone_now=true
    Wait(30)
    ...
    hide_old_blips()
    hideblips(join_faction)
    medics=faction.lsfd.medics
    skinshops={}--faction.lsfd.skinshops
    weaponshops={}--faction.lsfd.weaponshops
    carshops=faction.lsfd.carshops
    repairshops=faction.lsfd.repairshops
    mercenaries={}--faction.lsfd.mercenaries
    armorshops={}--faction.lsfd.armorshops
    clothes={}--faction.lsfd.clothes
    weapon_upgrade_shops={}--faction.lsfd.weapon_upgrade_shops
    special={}--faction.lsfd.special
...
switch_to.sspd=function()
    ...
    hide_old_blips()
    hideblips(join_faction)
    medics=faction.sspd.medics
    skinshops=faction.sspd.skinshops
    weaponshops=faction.sspd.weaponshops
    carshops=faction.sspd.carshops
    repairshops={}--faction.sspd.repairshops
    mercenaries={}--faction.sspd.mercenaries
    armorshops={}--faction.sspd.armorshops
    clothes={}--faction.sspd.clothes
    weapon_upgrade_shops={}--faction.sspd.weapon_upgrade_shops
    special={}--faction.sspd.special
...
join_faction[1]={func=switch_to.navy,x=-2349.6596679688,y=3268.3637695313,z=32.810760498047,sprite=60,color=26,markercolor={255, 200, 128, 128},name="Join Navy",models=SKINS.NAVY,removeweapons=true}
join_faction[2]={func=switch_to.military,x=-2345.8854980469,y=3268.2790527344,z=32.810749053955,sprite=60,color=52,markercolor={255, 200, 128, 128},name="Join Military",models=SKINS.MILITARY,removeweapons=true}
join_faction[3]={func=switch_to.noose,x=2475.6059570313,y=-384.12286376953,z=94.399314880371,sprite=60,color=29,markercolor={128, 128, 255, 128},name="Join SWAT",models=SKINS.NOOSE,components={[10]={0,0}},removeweapons=true}
join_faction[4]={func=switch_to.sahp,x=-448.84234619141,y=6013.0473632813,z=31.716390609741,sprite=60,color=39,markercolor={128, 128, 128, 128},name="Join SAHP",models=SKINS.SAHP,removeweapons=true}
join_faction[5]={func=switch_to.sspd,x=1853.5550537109,y=3689.3596191406,z=34.267066955566,sprite=60,color=5,markercolor={255, 255, 128, 128},name="Join SSPD",models=SKINS.SSPD,removeweapons=true}
join_faction[6]={func=switch_to.sapr,x=379.21508789063,y=792.20190429688,z=190.40716552734,sprite=60,color=2,markercolor={128, 255, 128, 128},name="Join SAPR",models=SKINS.SAPR,removeweapons=true}
join_faction[7]={func=switch_to.lspdheavy,x=446.41818237305,y=-986.47784423828,z=26.674211502075,sprite=60,color=3,markercolor={128, 128, 255, 128},name="Join LSPD Heavy Unit",models={1581098148},components={[9]=2},removeweapons=true}
join_faction[8]={func=switch_to.lspd,x=441.19030761719,y=-981.13079833984,z=30.689605712891,sprite=60,color=3,markercolor={128, 128, 255, 128},name="Join LSPD",models=SKINS.LSPD,components={[9]=0},removeweapons=true}
join_faction[9]={func=switch_to.fbiswat,x=156.0267791748,y=-741.1162109375,z=242.15208435059,sprite=60,color=4,markercolor={128, 128, 255, 128},name="Join FBI SWAT",models={-1145735340},removeweapons=true}
join_faction[10]={func=switch_to.fbi,x=115.88011169434,y=-748.78686523438,z=45.7516746521,sprite=60,color=4,markercolor={128, 128, 255, 128},name="Join FBI",models=SKINS.FBI,removeweapons=true}
join_faction[11]={func=switch_to.lost,x=984.76843261719,y=-91.682571411133,z=74.848892211914,sprite=378,color=62,markercolor={150, 150, 150, 128},name="Join Lost M.C.",models=SKINS.LOST}
join_faction[12]={func=switch_to.merc,x=484.337890625,y=-3052.2468261719,z=6.2286891937256,sprite=431,color=65,markercolor={150, 150, 200, 128},name="Join Mercs",models=SKINS.MERCS}
join_faction[13]={func=switch_to.anarchy,x=707.38952636719,y=-967.00140380859,z=30.412853240967,sprite=442,color=25,markercolor={150, 150, 200, 128},name="Join Anarchists",models=SKINS.ANARCHY}
join_faction[14]={func=switch_to.ballas,x=78.277633666992,y=-1974.7937011719,z=20.911375045776,sprite=491,color=83,markercolor={200, 0, 200, 128},name="Join Ballas",models=SKINS.BALLAS}
join_faction[15]={func=switch_to.fams,x=-11.148657798767,y=-1433.2587890625,z=31.116823196411,sprite=491,color=69,markercolor={0, 200, 0, 128},name="Join Families",models=SKINS.FAMILIES}
join_faction[16]={func=switch_to.vagos,x=967.79791259766,y=-1828.583984375,z=31.236526489258,sprite=491,color=46,markercolor={200, 200, 0, 128},name="Join Vagos",models=SKINS.VAGOS}
join_faction[17]={func=switch_to.salva,x=1230.8974609375,y=-1591.1851806641,z=53.820705413818,sprite=491,color=18,markercolor={0, 0, 200, 128},name="Join Salva",models=SKINS.SALVA}
join_faction[18]={func=switch_to.triads,x=-775.37646484375,y=-890.73687744141,z=21.605070114136,sprite=491,color=49,markercolor={200, 0, 0, 128},name="Join Triads",models=SKINS.TRIADS}
join_faction[19]={func=switch_to.armmob,x=-429.05123901367,y=-1728.0805664063,z=19.783840179443,sprite=491,color=39,markercolor={200, 0, 0, 128},name="Join Armenian Mobs",models=SKINS.MOBS}
join_faction[20]={func=switch_to.heister,x=134.90467834473,y=323.68109130859,z=116.72046661377,sprite=362,color=45,markercolor={125, 125, 125, 128},name="Join Heisters",models=SKINS.HEISTERS}
join_faction[21]={func=switch_to.cartel,x=1394.7595214844,y=1141.8107910156,z=114.61865997314,sprite=84,color=47,markercolor={125, 125, 125, 128},name="Join Cartel",models=SKINS.CARTEL}
join_faction[22]={func=switch_to.elite,x=-70.679946899414,y=359.08154296875,z=112.44521331787,sprite=432,color=4,markercolor={255, 255, 255, 128},name="Join Elite Security",models=SKINS.ELITE}
join_faction[23]={func=switch_to.paramedics,x=294.99197387695,y=-1447.7286376953,z=29.966623306274,sprite=61,color=8,markercolor={255, 200, 128, 128},name="Join Medics",models=SKINS.MEDICS,removeweapons=true}
join_faction[24]={func=switch_to.firefighters,x=199.41911315918,y=-1634.6833496094,z=29.80322265625,sprite=436,color=75,markercolor={255, 0, 0, 128},name="Join LSFD",models=SKINS.LSFD,removeweapons=true}
join_faction[25]={func=switch_to.detectives,x=452.10980224609,y=-984.57336425781,z=26.674234390259,sprite=60,color=3,markercolor={128, 128, 255, 128},name="Join Detectives",models=SKINS.DETECTIVES,removeweapons=true}
--join_faction[26]={func=switch_to.reporters,x=-598.76989746094,y=-929.91925048828,z=23.863523483276,sprite=564,color=3,markercolor={128, 128, 255, 128},name="Join Journalists",models=SKINS.REPORTERS,removeweapons=true}
--join_faction[27]={func=switch_to.coastguard,x=-1183.4830322266,y=-1773.9500732422,z=4.1827969551086,sprite=60,color=3,markercolor={128, 128, 255, 128},name="Join Coast Guard",models=SKINS.COASTGUARD,removeweapons=true}
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if player.civilian or player.is_cop then
            local your_car=nil
            if personal_responsibility==nil then
            ...
            if your_car~=nil then
                for ped in EnumeratePeds() do
                    if not DecorExistOn(ped,decor.civilianbecamecriminal) and not DecorExistOn(ped,decor.teamkiller_punished) then
                        ...
                        elseif personal_responsibility[ped] and GetEntityHealth(ped)<1 then
                            gang_member_killed_ped(ped,"betrayed")
                        end
                    end
                end
            else
            ...
            local timestamp=GetGameTimer()
            local hit_something,where=GetPedLastWeaponImpactCoord(you)
            if hit_something then
                local your_playerid=PlayerId()
                for i=your_playerid-1,0,-1 do
                    if NetworkIsPlayerActive(i) then
                        local ped=GetPlayerPed(i)
                        local name=dont_kill_those[GetEntityModel(ped)]
                        if name==nil then name=dont_kill_those[GetPedRelationshipGroupHash(ped)] end
                        if name then
                            local coords=GetEntityCoords(ped)
                            if #(coords-where)<1 then
                                personal_responsibility[ped]=timestamp
                                SetNotificationTextEntry("STRING");
                                AddTextComponentString("You hit "..name.." and now personally responsible for their life.")
                                DrawNotification(false, false)
                            end
                        end
                    end
                end
                for i=your_playerid+1,31 do
                    if NetworkIsPlayerActive(i) then
                        local ped=GetPlayerPed(i)
                        local name=dont_kill_those[GetEntityModel(ped)]
                        if name==nil then name=dont_kill_those[GetPedRelationshipGroupHash(ped)] end
                ...
                for ped,t in pairs(personal_responsibility) do
                    if t+20000<timestamp or not (dont_kill_those[GetEntityModel(ped)] or dont_kill_those[GetPedRelationshipGroupHash(ped)]) then
                        personal_responsibility[ped]=nil
                    end
                end
            end
        elseif faction~=1862763509 and dont_kill_those then --GetHashKey("PLAYER")
            ...
            if personal_responsibility~=nil then personal_responsibility=nil end
            pos=GetEntityCoords(PlayerPedId())
...
local function get_far_empty_veh(x,y,radius)
    ...
    local ret=0
    for veh in EnumerateVehicles() do
        if GetVehicleClass(veh)<10 and not IsEntityAMissionEntity(veh) and NetworkHasControlOfEntity(veh) then
        ...
        elseif (GetEntityModel(ped)==-294281201) then
            ...
            if veh~=nil then
                if garbage.truck_models[GetEntityModel(veh)] then
                    ...
                    local close_enough=(dx*dx+dy*dy+dz*dz<666.0)
                    for obj in EnumerateObjects() do
                        if garbage.trash_models[GetEntityModel(obj)] then
                            local garbage_pos=GetEntityCoords(obj)
                            local garbage_hash
...
local randommission={
 ...
 bases={
  ...
  { -- alley near scuba for crims
   x=-1263.3056640625,y=-1124.8481445313,z=7.5699214935303,r=120,
   pedspawns={
   {x=-1262.0018310547,y=-1123.7941894531,z=7.8964228630066,angle=107.46039581299},
   {x=-1277.9698486328,y=-1093.9398193359,z=7.2915940284729,angle=296.09912109375},
   {x=-1268.8790283203,y=-1096.6881103516,z=7.7856278419495,angle=113.046875},
   {x=-1267.5850830078,y=-1100.3352050781,z=7.7741394042969,angle=114.4282989502},
   {x=-1265.6674804688,y=-1108.7205810547,z=7.7463064193726,angle=111.21945953369},
   {x=-1270.0469970703,y=-1116.5971679688,z=7.2597789764404,angle=273.63021850586},
   {x=-1262.9898681641,y=-1117.5333251953,z=7.7266473770142,angle=107.93347930908},
   {x=-1265.3640136719,y=-1124.8916015625,z=7.5518321990967,angle=296.93133544922},
   {x=-1262.0162353516,y=-1133.4448242188,z=7.5827298164368,angle=291.92718505859},
   {x=-1258.7644042969,y=-1134.6469726563,z=7.6855907440186,angle=101.4384765625},
   {x=-1262.8348388672,y=-1139.0042724609,z=7.5349650382996,angle=246.05551147461},
   {x=-1252.671875,y=-1144.4879150391,z=8.5139627456665,angle=23.228303909302},
   {x=-1253.9543457031,y=-1140.8170166016,z=8.5678663253784,angle=200.44747924805},
   {x=-1257.2470703125,y=-1149.8659667969,z=7.6053137779236,angle=302.67614746094},
   {x=-1253.0578613281,y=-1151.6906738281,z=7.7111430168152,angle=111.39003753662},
   },
   vehspawn={x=-1266.1082763672,y=-1115.9373779297,z=7.2166090011597,angle=17.265037536621},
   vehdelivery={x=-1272.8074951172,y=-1096.4038085938,z=7.286190032959,angle=16.279748916626},
  },
 },
}
...
randommission.missions[1]=function()--movevehicle
    ...
    SetBlipColour(randommission.blip,2)
...
randommission.missions[6]=function()--free hostage
    ...
    while true do Wait(0)
        ...
        local vhealth=GetEntityHealth(vip)
        if IsPedFatallyInjured(pped) or IsPedFatallyInjured(vip) then
            ...
            DeletePed(vip)
            return
        elseif vhealth>100 and vhealth<200 then
            SetEntityHealth(vip,vhealth+1)
            Wait(500)
...
Citizen.CreateThread(function()
    ...
    {{x=-569.6,y=-927.70080566406,z=36.83358001709},{x=-575.14349365234,y=-940.65777587891,z=23.861577987671},2}, --weazel news
    --{{x=-1178.3,y=-1781.0843505859,z=4.2},{x=-1189.4404296875,y=-1783.0374755859,z=15.616714477539},1}, --useless teleport at coast guard base
    ...
    {{x=132.21673583984,y=-1462.0244140625,z=29.357046127319},{x=134.1340637207,y=-1463.7073974609,z=29.357027053833},0},--lucky plucker
...
fast_join_faction[GetHashKey("AMBIENT_GANG_LOST")]=switch_to.lost
--fast_join_faction[GetHashKey("COP")]=switch_to.cop
fast_join_faction[GetHashKey("AGGRESSIVE_INVESTIGATE")]=switch_to.merc
fast_join_faction[GetHashKey("MISSION2")]=switch_to.anarchy
fast_join_faction[GetHashKey("AMBIENT_GANG_BALLAS")]=switch_to.ballas
fast_join_faction[GetHashKey("AMBIENT_GANG_FAMILY")]=switch_to.fams
--fast_join_faction[GetHashKey("AMBIENT_GANG_MEXICAN")]=switch_to.vagos
fast_join_faction[GetHashKey("AMBIENT_GANG_SALVA")]=switch_to.salva
fast_join_faction[GetHashKey("AMBIENT_GANG_WEICHENG")]=switch_to.triads
fast_join_faction[GetHashKey("AMBIENT_GANG_MARABUNTE")]=switch_to.armmob
fast_join_faction[GetHashKey("MISSION3")]=switch_to.heister
--fast_join_faction[GetHashKey("AMBIENT_GANG_MEXICAN")]=switch_to.cartel
fast_join_faction[GetHashKey("MISSION4")]=switch_to.elite
...
AddEventHandler(event.savenquit_load,function(pos,model,components,props,health,armor,weapons,ammo,relationship,textures)
    ...
    if fjf~=nil then
     ...
     elseif allies_vagos[model]~=nil then
      ...
      for k,v in pairs(SKINS.COASTGUARD) do
       if model==v then
        switch_to.coastguard()
        break
       end
      end
      for k,v in pairs(SKINS.DETECTIVES) do
       if model==v then
            switch_to.detectives()
        break
       end
      end
      for k,v in pairs(SKINS.SSPD) do
...
local function ArrestedByNPCCop()
    ...
    elseif wanted == 5 then
        cost=math.floor(player.money*0.9)
...
Citizen.CreateThread(function()
    ...
    local insurgent7=0x48157027
    local squaddie2=74062924
    ...
    local swat2=GetHashKey("s_m_y_swat_02")
    ...
    local custom_dispatch_45={
     ...
     {mdl=granger,--FBI Grangler
      ...
      team={
       ...
       [1]={
        ...
        armor=100,
        wpns={--weapons
         [WEAPON.ADVANCEDRIFLE]={upgr={'scope'}},
        }
       },
       [2]={
        mdl=swatcopmodel,
     ...
     {mdl=squaddie2,--vehicle model
      rem=true,--remove anyway
      limits={
      {ped_count,heavyarmy,5},
      {veh_count,squaddie2,1},
      },
      team={
       [-1]={
        mdl=heavyarmy,
     ...
     {mdl=insurgent7,--vehicle model
      rem=true,--remove anyway
      limits={
      {ped_count,swat2,4},
      {veh_count,insurgent7,1},
      },
      clr1=152,--color 1
      clr2=152,--color 2
      team={--team
       ...
       [2]={--backright
        ...
        prop={{0,0,0}},--helmet
        wpns={--weapons
         [WEAPON.ADVANCEDRIFLE]={upgr={'scope','flashlight','silencer'}}
        }
       },
      }
     },
...
[59]={x=2936,y=5326,z=100},
[60]={x=-1053,y=-1874,z=7},
[61]={x=1089,y=-2160,z=30},
[62]={x=496,y=-1466,z=9},

[63]={x=1086,y=-2004,z=31},
[64]={x=1086,y=-2005,z=31},
[65]={x=1087,y=-2004,z=31},
[66]={x=1085,y=-2005,z=31},
[67]={x=1111,y=-2010,z=32},
[68]={x=1087,y=-2003,z=31},
[69]={x=1086,y=-2003,z=31},
[70]={x=1085,y=-2004,z=31},

[71]={x=2858,y=333,z=2},
[72]={x=-40,y=821,z=230},
[73]={x=-39,y=822,z=230},

[74]={x=2682,y=4875,z=31},
[75]={x=2682,y=4875,z=32},

[76]={x=71,y=3604,z=38},
[77]={x=71,y=3604,z=39},
[78]={x=-195,y=3918,z=33},
[79]={x=-203,y=3939,z=33},

[80]={x=1787,y=3892,z=33},
[81]={x=1787,y=3892,z=34},

[82]={x=1232,y=4363,z=34},
[83]={x=1239,y=4350,z=33},

[84]={x=-2168,y=5185,z=14},--avi
[85]={x=-2168,y=5184,z=14},

[86]={x=2048,y=3565,z=39},
[87]={x=2539,y=6618,z=1},
[88]={x=2981,y=6373,z=0},

[89]={x=2941,y=781,z=25},
[90]={x=2512,y=-1222,z=1},
[91]={x=2511,y=-1222,z=1},
[92]={x=2333,y=-2104,z=4},
[93]={x=2325,y=-2103,z=4},

[94]={x=1110,y=-1251,z=19},
[95]={x=1110,y=-1251,z=20},

[96]={x=-1241,y=-1785,z=1},

[97]={x=-1329,y=-1697,z=1},
[98]={x=-1348,y=-1660,z=1},
[99]={x=-1422,y=-1539,z=1},
[100]={x=-1464,y=-1358,z=2},
[101]={x=-1493,y=-1285,z=1},

[102]={x=-1538,y=-1217,z=1},
[103]={x=-1538,y=-1216,z=1},
[104]={x=-1572,y=-1133,z=2},
[105]={x=-1572,y=-1132,z=2},
[106]={x=-1785,y=-888,z=5},
[107]={x=-1788,y=-867,z=6},
[108]={x=-1819,y=-907,z=1},
[109]={x=-1840,y=-787,z=6},
[110]={x=-1840,y=-786,z=6},
[111]={x=-1839,y=-800,z=5},
[112]={x=-1839,y=-799,z=5},
[113]={x=-1871,y=-772,z=5},
[114]={x=-1888,y=-732,z=5},
[115]={x=-1955,y=-660,z=5},
[116]={x=-1923,y=-700,z=5},
[117]={x=-2044,y=-553,z=6},
[118]={x=-2066,y=-550,z=4},
[119]={x=-2769,y=-63,z=5},
[120]={x=-2885,y=-19,z=5},
[121]={x=-2883,y=-32,z=3},
[122]={x=-2946,y=-7,z=5},
[123]={x=-2953,y=4,z=6},
[124]={x=-3283,y=985,z=3},
[125]={x=-3273,y=1015,z=4},
[126]={x=-3275,y=1017,z=4},
[127]={x=-3257,y=1103,z=1},
[128]={x=-3243,y=1197,z=2},
[129]={x=-3228,y=1235,z=2},
[130]={x=2447,y=3778,z=40},
[131]={x=2477,y=3761,z=41},
[132]={x=2483,y=3726,z=42},
[133]={x=337,y=-1086,z=28},
},
blacklist2={
...
Citizen.CreateThread(function()
    while true do
        ...
        elseif IsControlJustPressed(0,86) then
            pos=GetEntityCoords(ped)
            -- if IsObjectNearPoint(-509973344,pos.x,pos.y,pos.z,1.1) then --medkit
                -- if forced_max_health~=nil then
                    -- SetEntityHealth(ped,forced_max_health)
                -- else
                ...
                -- end
                -- Wait(1000)
            -- elseif IsObjectNearPoint(-1610165324,pos.x,pos.y,pos.z,1.1) --water fire extinguisher model
            -- or IsObjectNearPoint(-875057463,pos.x,pos.y,pos.z,1.1) --CO2 fire extinguisher model
            -- then
                -- GiveWeaponToPed(ped,101631238,1000,false,true)
                -- --print("near fire extinguisher")
                -- Wait(1000)
            -- else
                -- Wait(100)
            -- end
...
special_abilities.reporter=function()
    local myped=PlayerPedId()
    local pos=GetEntityCoords(myped)
    if IsPedInAnyVehicle(myped) then
        local veh=GetVehiclePedIsUsing(myped)
...
Citizen.CreateThread(function()
    ...
    local accel=.00000001
    local speed1=.0000003
    local speedmult=1.966/.0000003 -- 1.9385/.0000003
    local speed2=speed1*speedmult
    local explosion_detected=0
    local health=1000
    local offset=.0
    local r=6550.0
    local control_cooldown=0
    ...
    local function in_submarine(pos)
        return math.abs(pos.z+66)<12 and math.abs(pos.y-4861)<47 and math.abs(pos.x-514)<5
        --and pos.y<4908 and pos.y>4814 and pos.z<-60 and pos.z>-73
    end
    local function in_control_room(pos)
        return math.abs(pos.x-514)<4 and math.abs(pos.y-4838.5)<5 and math.abs(pos.z+62.0)<1.1
    end
    local function near_reactor(pos,angle)
        -- x=515.00836181641,y=4874.7548828125,z=-68.99089050293 near door and control
        return pos.y>4874.07 and pos.y<4878.17 and math.abs(pos.z+68.99)<1.1
    end
    local function near_front_diesel(pos,angle)
        -- x=515.71508789063,y=4839.1958007813,z=-68.990600585938 near panel
        return math.abs(pos.x-515.715)+math.abs(pos.y-4839.1958)+math.abs(pos.z+68.99)<1.1 and (angle>40 and angle<140)
        -- x=511.564453125,y=4840.9975585938,z=-68.486167907715 near wall panel
        --return pos.y>4874.07 and pos.y<4878.17 math.abs(pos.z+68.99)<1.1
    end
    local function near_back_diesel(pos,angle)
        -- x=513.48828125,y=4888.2724609375,z=-66.19457244873 near control
        return math.abs(pos.x-513.49)+math.abs(pos.y-4888.27)+math.abs(pos.z+66.194)<1.1 and (angle>220 and angle<320)
    end
    local function near_heat_exchanger_control(pos,angle)
        -- x=513.19866943359,y=4887.3359375,z=-68.989303588867
        return math.abs(pos.x-513.198)+math.abs(pos.y-4887.3359)+math.abs(pos.z+68.9893)<1.1 and (angle>270 or angle<45)
    end
    local function near_turbine_control(pos,angle)
        -- x=515.01568603516,y=4892.1333007813,z=-69.003456115723
        return math.abs(pos.x-515)+math.abs(pos.y-4892.1333)+math.abs(pos.z+69)<1.1 and (angle>315 or angle<45)
    end
    local function near_back_control(pos,angle)
        -- x=517.329,y=4906.4736,z=-66.143127
        return math.abs(pos.x-517.329)+math.abs(pos.y-4906.4736)+math.abs(pos.z+66.143127)<1.1 and (angle>315 or angle<45)
    end
    local function near_submarine(pos)
        return math.abs(pos.x-x)+math.abs(pos.y-y)<140.0 and math.abs(pos.z-z-2)<19.7
    end
    ...
    local seats={
        {x=511.5,y=4834.07,z=-63.1,angle=117.4121},
        {x=511.5,y=4835.25,z=-63.1,angle=90.0},--standard
        {x=511.5,y=4836.605,z=-63.1,angle=71.795},
        {x=511.5,y=4838.0,z=-63.1,angle=90.0},
        {x=511.5,y=4839.31,z=-63.1,angle=71.795},
        {x=511.5,y=4840.62,z=-63.1,angle=90.0},--recorded myself
        
        {x=516.77,y=4836.92,z=-63.1,angle=270.0},--standard
        {x=516.77,y=4838.154785,z=-63.1,angle=270.0},
        
        --{x=513.13330078125,y=4833.640625,z=-63.1,angle=215.36225891},
        {x=513.02,y=4833.76,z=-63.1,angle=215.36225891},
    }
    local delete_when_destroyed={
    [1183164986]=true,--big thing(two small things in opposite sides of control room)
    --control panels
    [-220397414]=true,
    [847085530]=true,
    [541317991]=true,
    ...
    AddEventHandler(event.submarine,function(s,o,d)
        speed1,offset,z=s,o,d
        speed2=speed1*speedmult
    end)
    ...
    AddEventHandler(event.submarine_destroy,function()
        if GetEntityModel(submarine_visual)==visual_model then
            ...
            SetEntityLodDist(submarine_visual,lod_dist)
            health=0
            if in_submarine(GetEntityCoords(PlayerPedId())) then
                SetBlackout(true)
            end
        end
    end)
    ...
    local function fpsfix_toggle()
        --Citizen.InvokeNative(0x80ECBC0C856D3B0B,fpsfix)
        fpsfix=not fpsfix
        SetFarShadowsSuppressed(fpsfix)
        DisableVehicleDistantlights(fpsfix)
        DisplayDistantVehicles(not fpsfix)
        print(fpsfix and "fpsfix enabled" or "fpsfix disabled")
        TriggerEvent("chatMessage","FPS fix",{255,255,255},fpsfix and "enabled" or "disabled")
        if not in_submarine(GetEntityCoords(PlayerPedId())) then
            SetBlackout(fpsfix)
        end
    end
    RegisterCommand("fixfps",fpsfix_toggle,false)
    RegisterCommand("fpsfix",fpsfix_toggle,false)
    while true do
        ...
        local solidify=(IsPedRagdoll(ped) or IsPedSwimming(ped) or IsPedFalling(ped) or vel.z<-2)
        --drawtext("DeltaZ: "..tostring(pos.z-z-2))
        
        FreezeEntityPosition(submarine_collision,solidify)--or #(GetEntityRotationVelocity(submarine_collision))>.01
        SetEntityCoords(submarine_collision,x,y,z,false,false,false,false)
        FreezeEntityPosition(submarine_visual,solidify)--or #(GetEntityRotationVelocity(submarine_visual))>.01
        ...
        if health>0 then
            if math.abs(timestamp-explosion_detected)>200 and IsExplosionInSphere(-1,x,y,z,70.0) then
                explosion_detected=timestamp
                health=health-1
                if IsExplosionInSphere(-1,x,y,z,50.0) then
                    health=health-9
                    if IsExplosionInSphere(-1,x,y,z,30.0) then
                        health=health-15
                    end
                end
                print("sub health="..health)
                if health<0 then health=0 end
                if health==0 then
                    TriggerServerEvent(event.submarine_destroy)
                end
            end
        elseif in_submarine(pos) then
            local handle,obj=FindFirstObject()
            local loop=(handle~=-1)
            while loop do
                local pos=GetEntityCoords(obj)
                if in_submarine(pos) and delete_when_destroyed[GetEntityModel(obj)] then
                    SetEntityAsMissionEntity(obj)
                    DeleteObject(obj)
                end
        ...
        if near_reactor(pos,heading) then
            drawtext("reactor")
        elseif near_front_diesel(pos,heading) then
            drawtext("front diesel")
        elseif near_back_diesel(pos,heading) then
            drawtext("back diesel")
        elseif near_heat_exchanger_control(pos,heading) then
            drawtext("heat exchanger")
        elseif near_turbine_control(pos,heading) then
            drawtext("turbine")
        elseif near_back_control(pos,heading) then
            drawtext("idk something")
        end
        if rocket_wait then
        ...
        elseif periscope_camera then
            ...
            local forward
            _,forward=GetCamMatrix(periscope_camera)
            if health>0 and IsDisabledControlJustPressed(0,24) then
        ...
        elseif IsControlJustPressed(0,86) or IsControlJustPressed(0,75) then
            if in_submarine(pos) then
                if math.abs(pos.x-periscope_in.x)+math.abs(pos.y-periscope_in.y)<4.0 and math.abs(pos.z-periscope_in.z)<.2 then --control
                    ...
                    local rot=GetGameplayCamRot(2)
                    SetCamRot(periscope_camera,rot.x,rot.y,rot.z+angle+90.0,2)
                elseif in_control_room(pos) then
                    local sit_scenario="PROP_HUMAN_SEAT_BENCH"
                    --local seat=GetClosestObjectOfType(pos.x,pos.y,pos.z,2.0,478236466, false, false, false)print("seat "..seat)
                    --if seat~=0 then
                        --if IsPedUsingScenario(ped,sit_scenario) then
                        --    ClearPedTasks(ped)
                        --else
                            --local seat_pos=GetEntityCoords(seat)
                            --TaskStartScenarioAtPosition(ped, sit_scenario, seat_pos.x, seat_pos.y, seat_pos.z+.5, GetEntityHeading(seat)+180.0, -1, true, true)
                        --end
                    --end
                    local dx,dy,dz
                    local dist
                    ...
                    if IsPedUsingScenario(ped,sit_scenario) then
                        ClearPedTasks(ped)
                        SetEntityCoords(ped,(pos.x-514)*.7+514,pos.y,-62.58)
                    else
                        for i=0,31 do
                            if NetworkIsPlayerActive(i) then
                                local ppos=GetEntityCoords(GetPlayerPed(i))
                                if in_control_room(ppos) then
                                    for k,v in pairs(seats) do
                                        if math.abs(ppos.x-v.x)+math.abs(ppos.y-v.y)+math.abs(ppos.z-v.z)<.2 then
                                            v.occupied=true
                                            break
                                        end
                                    end
                        ...
                        if mindist<1.0 then
                            TaskStartScenarioAtPosition(ped, sit_scenario, closest.x, closest.y, closest.z, closest.angle, -1, true, true)
                        end
                    end
                else
            ...
            elseif near_submarine(pos) then
                local front_hatch={x=x+s*10.218,y=y-c*10.218,z=z+11.0}
        ...
        elseif health>0 and in_control_room(pos) then
            drawtext("Speed: "..tostring(math.floor(speed2/.514)).."kn\nDepth: "..tostring(math.floor(10-z)).."m\nHull:"..(health*.1).."%")
            if control_cooldown<timestamp then
                local update=false
                ...
                if update then
                    control_cooldown=timestamp+150
                    TriggerServerEvent(event.submarine,speed1,offset,z)
                end
            end
            if blip==nil then
