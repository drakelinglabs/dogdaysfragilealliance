...
WEAPON.ASSAULTRIFLEMK2=961495388
WEAPON.CARBINERIFLEMK2=0xFFFFFFFFFAD1F1C9
WEAPON.COMBATMGMK2=0xFFFFFFFFDBBD7280
WEAPON.HEAVYSNIPERMK2=177293209
WEAPON.PISTOLMK2=0xFFFFFFFFBFE256D4
WEAPON.SMGMK2=2024373456
...
Wait(5000)
    while true do
		...
		if player_is_cop then
	        SetTextComponentFormat("STRING")
	        AddTextComponentString("Press ~INPUT_VEH_HEADLIGHT~ to arrest surrending player.")
	        DisplayHelpTextFromStringLabel(0,0,1,-1)
	        Wait(600000)
		else
	        SetTextComponentFormat("STRING")
...
AddEventHandler("fragile-alliance:playerspawn",function(ped)
    --local ped = PlayerPedId()
        ...
        SetPedRandomComponentVariation(ped, false)
        --if garages then
        if relationship_friend==GetHashKey("COP") then
            SetEntityCoords(ped,441.19030761719,-981.13079833984,30.689605712891)
...
weapon_upgrades.extendedmag[WEAPON.COMPACTRIFLE]=0xC607740E --drum
...
weapon_upgrades.luxe={}
weapon_upgrades.luxe[WEAPON.PISTOL]=0xD7391086
weapon_upgrades.luxe[WEAPON.PISTOL50]=0x77B8AB2F
weapon_upgrades.luxe[WEAPON.COMBATPISTOL]=0xC6654D72
weapon_upgrades.luxe[WEAPON.APPISTOL]=0x9B76C72C
weapon_upgrades.luxe[WEAPON.SNSPISTOL]=0x8033ECAF
weapon_upgrades.luxe[WEAPON.HEAVYPISTOL]=0x7A6A7B7B
weapon_upgrades.luxe[WEAPON.SMG]=0x27872C90
weapon_upgrades.luxe[WEAPON.MICROSMG]=0x487AAE09
weapon_upgrades.luxe[WEAPON.ASSAULTSMG]=0x278C78AF
weapon_upgrades.luxe[WEAPON.MG]=0xD6DABABE
weapon_upgrades.luxe[WEAPON.COMBATMG]=0x92FECCDD
weapon_upgrades.luxe[WEAPON.ASSAULTRIFLE]=0x4EAD7533
weapon_upgrades.luxe[WEAPON.CARBINERIFLE]=0xD89B9658
weapon_upgrades.luxe[WEAPON.ADVANCEDRIFLE]=0x377CD377
weapon_upgrades.luxe[WEAPON.SPECIALCARBINE]=0x730154F2
weapon_upgrades.luxe[WEAPON.BULLPUPRIFLE]=0xA857BC78
weapon_upgrades.luxe[WEAPON.SAWNOFFSHOTGUN]=0x85A64DF9
weapon_upgrades.luxe[WEAPON.PUMPSHOTGUN]=0xA2D79DDB
weapon_upgrades.luxe[WEAPON.SNIPERRIFLE]=0x4032B5E7

--mk2
weapon_upgrades.silencer[WEAPON.PISTOLMK2]=0x65EA7EBB
weapon_upgrades.flashlight[WEAPON.PISTOLMK2]=0x43FD595B
weapon_upgrades.scope[WEAPON.PISTOLMK2]=0xFFFFFFFF8ED4BB70

weapon_upgrades.silencer[WEAPON.SMGMK2]=0xFFFFFFFFC304849A
weapon_upgrades.scope[WEAPON.SMGMK2]=0xFFFFFFFF9FDB5652
weapon_upgrades.flashlight[WEAPON.SMGMK2]=0x7BC4CDDC

weapon_upgrades.grip[WEAPON.ASSAULTRIFLEMK2]=0xFFFFFFFF9D65907A
weapon_upgrades.silencer[WEAPON.ASSAULTRIFLEMK2]=0xFFFFFFFFA73D4664
weapon_upgrades.scope[WEAPON.ASSAULTRIFLEMK2]=0xFFFFFFFFC66B6542
weapon_upgrades.flashlight[WEAPON.ASSAULTRIFLEMK2]=0x7BC4CDDC

weapon_upgrades.grip[WEAPON.CARBINERIFLEMK2]=0xFFFFFFFF9D65907A
weapon_upgrades.silencer[WEAPON.CARBINERIFLEMK2]=0xFFFFFFFF837445AA
weapon_upgrades.scope[WEAPON.CARBINERIFLEMK2]=0x49B2945
weapon_upgrades.flashlight[WEAPON.CARBINERIFLEMK2]=0x7BC4CDDC

weapon_upgrades.grip[WEAPON.COMBATMGMK2]=0xFFFFFFFF9D65907A
weapon_upgrades.scope[WEAPON.COMBATMGMK2]=0x3F3C8181

weapon_upgrades.silencer[WEAPON.HEAVYSNIPERMK2]=0xFFFFFFFFAC42DF71
weapon_upgrades.scope[WEAPON.HEAVYSNIPERMK2]=0xFFFFFFFFBC54DA77
...
local gangwar={
    top={
        [FACTION.FBI]={color={30,119,166},name="FBI",score=0},
...
{x=140.86126708984,y=-2204.3400878906,z=4.6880211830139,name="Cheap proxmine",sprite=152,color=0,cost=800,weapon={WEAPON.PROXMINE}, --torture
                                                                                   ammo={           1}},
{x=-935.54614257813,y=-2935.3081054688,z=13.945076942444,name="Free pistol",sprite=156,color=0,cost=0,weapon={WEAPON.MARKSMANPISTOL}, --airport
                                                                                   ammo={           1}},
                ...
                if (square<4) and (not isDead) then
                    ...
                    elseif IsControlPressed(0, 86) then
                        if player_money>=v.cost then
                            ...
                            if v.luxe then
                                GiveWeaponComponentToPed(GetPlayerPed(-1),v.weapon[i],weapon_upgrades.luxe[v.weapon[i]])
                            else
                                RemoveWeaponComponentFromPed(GetPlayerPed(-1),v.weapon[i],weapon_upgrades.luxe[v.weapon[i]]);
                            end
...
AddEventHandler(event.teleport,function(x,y,z)
...
RegisterNUICallback('shop_buy_weapon', function(data, cb)
    ...
    if player_money~=nil and player_money>=data.cost then
        player_money=player_money-data.cost
        removemoney(player_money,data.cost)
        TriggerServerEvent(event.buy,data.cost)
        if data.weapon=="bodyarmor" then
            SetPedArmour(GetPlayerPed(-1), 100);
        else
            GiveWeaponToPed(GetPlayerPed(-1), GetHashKey("WEAPON_"..data.weapon), 1000, false, true)
        end
        SetTextComponentFormat("STRING")
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for propertyname,v in pairs(apartments) do
            if math.abs(pos.x-v.zone.x)+math.abs(pos.y-v.zone.y)<v.zone.r and math.abs(pos.z-v.zone.z)<v.zone.h -- safe zone
            then
                ...
                then
                    SetEntityCoords(GetPlayerPed(-1),v.entrance.x,v.entrance.y,v.entrance.z)
                end
                ...
                if v.heal~=nil then
                    ...
                    then
                        ...
                        then
                            ...
                            local maxhealth=GetPedMaxHealth(ped)
                            if GetEntityHealth(ped)<maxhealth then
                                SetEntityHealth(ped,maxhealth) end
                        end
                    end
                end
                ...
                if v.weaponshop~=nil then
                    ...
                    then
                        ...
                        then  -- открыть оружейную
                            SendNUIMessage({
                            enable_weapon_shop = propertyname
                            })
                    ...
                    then
                        SendNUIMessage({
                        disable_weapon_shop = propertyname
                        })
                        SetNuiFocus(false,false)
                        openarmory=0
                    end
                ...
                for i=0,31 do
                    ...
                    --print("invisible")
                    if another_player~=0 then
                        if another_player==ped then
                            SetPlayerVisibleLocally(i, true)
                            SetEntityCollision(another_player,true,true)
                        else
                            --print(another_player.."~="..ped)
        ...
        if im_in then
            ...
            if openarmory==1 then
                SendNUIMessage({
                disable_weapon_shop = 'altahotel'
                })
                SendNUIMessage({
                disable_weapon_shop = 'dno_1'
                })
                SendNUIMessage({
                disable_weapon_shop = 'bomjatnya_1'
                })
                SendNUIMessage({
                disable_weapon_shop = 'norm_1'
                })
                SetNuiFocus(false,false)
                openarmory=0
            end
            if openstash==1 then
                SendNUIMessage({
                disable_stash = 'altahotel'
                })
                SendNUIMessage({
                disable_stash = 'dno_1'
                })
                SendNUIMessage({
                disable_stash = 'bomjatnya_1'
                })
                SendNUIMessage({
                disable_stash = 'norm_1'
                })
                SetNuiFocus(false,false)
                openstash=0
            end
            ...
            for propertyname,v in pairs(apartments) do
                ...
                then
                    local playerfaction=GetPedRelationshipGroupHash(GetPlayerPed(-1))
                    if v.factions_allowed==nil or v.factions_allowed[playerfaction]
                    then
                        if v.cost~=nil
                        then
                            if v.firsttimecheck then
...
Wait(5000)
 ...
 criminal.carshops={
    ...
    {x=128.1043548584,y=-2197.1125488281,z=6.0333247184753,angle=320.2,sprite=477,name="Smallest truck",cost=20000,cars={-884690486}},
    {x=409.13833618164,y=-2069.9428710938,z=21.3864402771,angle=140.57992553711,sprite=477,name="Getaway van",cost=8000,cars={904750859,-1050465301}},
    {x=1031.9289550781,y=-1963.2841796875,z=30.529121398926,angle=94.166656494141,sprite=477,color=2,name="Upgraded getaway van",cost=20000,cars={-2052737935}},
                    ...
                    elseif IsControlPressed(0, 86) then
                        if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
                            ...
                            elseif player_money>=v.cost then
                                ...
                                if v.color1~=nil then
                                    if type(v.color1)=='table' then
                                        custom1=true
                                    else
                                        standard1=true
                                    end
                                end
                                if v.color2~=nil then
                                    if type(v.color2)=='table' then
                                        custom2=true
                                    else
                                        standard2=true
                                    end
                                ...
                                if standard1 or standard2 then
                                    ...
                                    if standard1 then color1=v.color1 end
                                    if standard2 then color2=v.color2 end
                                    SetVehicleColours(veh,color1,color2)
                                end
                                if custom1 then SetVehicleCustomPrimaryColour(veh,v.color1[1],v.color1[2],v.color1[3]) end
                                if custom2 then SetVehicleCustomSecondaryColour(veh,v.color2[1],v.color2[2],v.color2[3]) end
...
local repair_wheels,repair_windows,repair_engine,repair_body,repair_paint,repair_full=1,2,4,8,16,31;
 criminal.repairshops={
    {x=888.95196533203,y=-889.42864990234,z=26.414888381958,name="Repair engine and wheels",color=2,sprite=402,cost=2000,flags=repair_wheels+repair_windows+repair_engine},
    {x=-31.853324890137,y=-1090.5874023438,z=26.422239303589,name="Repair engine and wheels",color=2,sprite=402,cost=2000,flags=repair_wheels+repair_windows+repair_engine},
    {x=-27.108713150024,y=-1493.427734375,z=30.063791275024,name="Repair engine and wheels",color=69,sprite=402,cost=2300,factionreq={FACTION.FAMILIES,15000},flags=repair_wheels+repair_windows+repair_engine},
    {x=967.56573486328,y=-1812.7857666016,z=30.833524703979,name="Repair engine and wheels",color=46,sprite=402,cost=3000,factionreq={FACTION.VAGOS,100000},flags=repair_wheels+repair_windows+repair_engine},
    {x=-450.90884399414,y=-1691.4396972656,z=18.660259246826,name="Repair engine and wheels",color=39,sprite=402,cost=1500,factionreq={FACTION.MOBS,100000},flags=repair_wheels+repair_windows+repair_engine},
    {x=471.21493530273,y=-578.73657226563,z=28.49973487854,name="Full repair",color=1,sprite=446,cost=5000,flags=repair_full},
    ...
    while true do
        ...
        for k,v in pairs(garages) do
            ...
            if square<100 then
                ...
                if (square<v.radius) and (not isDead) then
                    if IsControlPressed(0, 86) then
...
table.insert(join_faction,{func=switch_to_cop,x=441.19030761719,y=-981.13079833984,z=30.689605712891,sprite=60,color=3,markercolor={128, 128, 255, 128},cost=25000,name="Join FBI",models=SKINS.FBI,removeweapons=true})
table.insert(join_faction,{func=switch_to_cop,x=115.88011169434,y=-748.78686523438,z=45.7516746521,sprite=60,color=3,markercolor={128, 128, 255, 128},cost=25000,name="Join FBI",models=SKINS.FBI,removeweapons=true})
...
carjack.mission=function()
    ...
    while GetVehicleEngineHealth(carjack.mission_veh)>1.0 do
        ...
        if IsPedInAnyVehicle(GetPlayerPed(-1)) and GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1)))==carjack.mission_plate then
            BeginTextCommandPrint("STRING");
    ...
    while true do
        ...
        elseif pilot.autopilot_old and not pilot.autopilot then
            ...
            if dist<300.0 and speed<1.0 then
                ...
                aps=aps..(math.floor(dist*.01)*.1).."km\n"..math.floor(speed*3.6).."km/h\n"
...
AddEventHandler(event.savenquit_load,function(pos,model,components,props,health,armor,weapons,ammo,relationship)
    while GetEntityModel(GetPlayerPed(-1))~=model do Wait(50) end
    Wait(600)
    ...
    Wait(100)
    if is_model_scuba(model) then givescubagear(GetPlayerPed(-1)) end
end)
        ...
        if player_is_cop then
            ...
            for i = 0, 31 do
                if NetworkIsPlayerActive(i) and GetPlayerWantedLevel(i)>0 then
                        local b=blips[i]
                        local pos=GetPlayerWantedCentrePosition(i)
                        if b==nil then
                            b=AddBlipForRadius(pos.x,pos.y,pos.z,100.0)
                            blips[i]=b
                            SetBlipDisplay(b, 2)
                            SetBlipSprite(b,9)
                            SetBlipColour(b,3)
                            ...
                            SetBlipCoords(b,pos.x,pos.y,pos.z)
                        end
            ...
            if GetPlayerWantedLevel(PlayerId())>0 then
                local pos=GetPlayerWantedCentrePosition(PlayerId())
                if blip==nil then
                    blip=AddBlipForRadius(pos.x,pos.y,pos.z,100.0)
                    SetBlipDisplay(blip, 2)
                    SetBlipSprite(blip,9)
                    SetBlipColour(blip,3)
                    SetBlipAlpha(blip,100)
...
local surrender={
 dict_surrender='random@arrests@busted',anim_surrender='idle_a',
 -- dict_walking='mp_arresting',anim_walking='walk',
 -- vehicle_models={
  -- [-1627000575]=true,
  -- [1912215274]=true,
  -- [-1973172295]=true,
...
-- surrender.closest_car=function()
    -- local pos=GetEntityCoords(GetPlayerPed(-1))
    -- local closest
    -- local closest_sdist=50.0
    -- for veh in EnumerateVehicles() do
        -- if surrender.vehicle_models[GetEntityModel(veh)] and IsVehicleDriveable(veh) then
            -- local vpos=GetEntityCoords(veh)
            -- -- local dx,dy,dz=pos.x-vpos.x,pos.y-vpos.y,pos.z-vpos.z
            -- -- local squaredist=dx*dx+dy*dy+dz*dz
            -- local squaredist=CalculateTravelDistanceBetweenPoints(pos.x,pos.y,pos.z,vpos.x,vpos.y,vpos.z)
            -- if squaredist<closest_sdist then
                -- closest_sdist=squaredist
                -- closest=veh
            -- end
        -- end
    -- end
    -- return closest
-- end
surrender.handsup=function()
    TaskPlayAnim(GetPlayerPed(-1), surrender.dict_surrender, surrender.anim_surrender, 2.0, -8.0, -1, 0, 0, 0, 0, 0)
    SetNotificationTextEntry("STRING");
    AddTextComponentString("Hold ~g~SPRINT ~s~button to stop surrendering.")
    DrawNotification(false, false);
    if IsControlPressed(0,21) then surrender.in_progress=false end
    if not surrender.in_progress then return end
    Wait(1000)
    if IsControlPressed(0,21) then surrender.in_progress=false end
    if not surrender.in_progress then return end
    Wait(1000)
...
-- surrender.handcufs=function()
    -- --SetEnableHandcuffs(GetPlayerPed(-1),true)
    -- --SetPedToRagdoll(GetPlayerPed(-1), 1000, 1000, 3, true, true, false);
    
    -- SetNotificationTextEntry("STRING")
    -- AddTextComponentString("Handcuffed.")
    -- DrawNotification(false, false)
    -- Wait(500)
-- end
-- surrender.disablecontrols=function()
...
-- surrender.drivetopolicestation=function()
    -- --if IsPedInVehicle(surrender.closest_cop,surrender.veh,false) then
    -- --TaskEnterVehicle(surrender.closest_cop,surrender.veh,-1,-1,1.0,1,0)
    -- --else
    -- TaskVehicleDriveToCoord(surrender.closest_cop,surrender.veh,
    -- 431.58267211914,-998.01678466797,25.357425689697, 70.0, 1.0, GetEntityModel(surrender.veh), 786603, 1.0, true)
    -- --end
    -- SetPedKeepTask(surrender.closest_cop, true);
    -- TaskEnterVehicle(GetPlayerPed(-1),surrender.veh,-1,1,1.0,1,0)
    -- local dx,dy,dz=pos.x-431.58267211914,pos.y+998.01678466797,pos.z-25.357425689697
    -- local sdist=dx*dx+dy*dy+dz*dz
    -- if sdist<100.0 then surrender.arrested() end
    -- Wait(1000)
-- end

-- surrender.arrested=function()
    -- if surrender.wanted_level==1 then
    -- TriggerServerEvent(event.buy,math.floor(player_money*0.4))
    -- elseif surrender.wanted_level==2 then
    -- TriggerServerEvent(event.buy,math.floor(player_money*0.5))
    -- elseif surrender.wanted_level==3 then
    -- TriggerServerEvent(event.buy,math.floor(player_money*0.65))
    -- elseif surrender.wanted_level==4 then
    -- TriggerServerEvent(event.buy,math.floor(player_money*0.80))
    -- elseif surrender.wanted_level==5 then
    -- TriggerServerEvent(event.buy,math.floor(player_money*0.95))
    -- end
    -- TriggerClientEvent(event.savenquit_none,math.random())
    -- surrender.in_progress=false
-- end

-- surrender.getincar=function()
    -- --SetEnableHandcuffs(GetPlayerPed(-1),true)
    -- SetCurrentPedWeapon(GetPlayerPed(-1), GetHashKey("WEAPON_UNARMED"), true)
    -- SetNotificationTextEntry("STRING")
    -- AddTextComponentString("Get in car.")
    -- SetPlayerWantedLevel(PlayerId(),0,false)
    -- --ForcePedMotionState(GetPlayerPed(-1), GetHashKey("motionstate_walk"), false, false, false);
    -- DrawNotification(false, false)
    -- local veh=surrender.closest_car()
    -- if veh==nil then
        
    -- elseif veh~=surrender.veh then
        -- print(GetDisplayNameFromVehicleModel(GetEntityModel(veh)).." veh="..veh)
        -- --TaskEnterVehicle(GetPlayerPed(-1),veh,0,i,2.0,16,0)
        -- TaskEnterVehicle(GetPlayerPed(-1),veh,-1,1,1.0,1,0)
        -- surrender.veh=veh
    -- end
            -- --StopAnimTask(GetPlayerPed(-1), dictsurrender, "handsup_standing_base", 1.0)
            -- --ResetPedMovementClipset(GetPlayerPed(-1), 0.0);
    -- Wait(1000)
-- end
surrender.loop_cops=function(player)
    local pos=GetEntityCoords(player)
    local cop_list={}
    local i=0
    local number_of_cops=0
    local closest_cop_index=0
    local closest_sdist=1000000.0
    for cop in EnumeratePeds() do
        if cop~=player and IsPedInCombat(cop,player) and GetPedRelationshipGroupHash(cop)==GetHashKey("COP") then
            --local cpos=GetEntityCoords(cop)
            --local dx,dy,dz=pos.x-cpos.x,pos.y-cpos.y,pos.z-cpos.z
            --local squaredist=dx*dx+dy*dy+dz*dz
            --if squaredist<16.0 then
            --    number_of_cops=number_of_cops+1
            --end
            --i=i+1
            --cop_list[i]=cop
            --if squaredist<closest_sdist then
        ...
        --end
    --end
    -- surrender.closest_cop=cop_list[closest_cop_index]
    -- surrender.closest_cop_sdist=closest_sdist
    -- surrender.cops_around=number_of_cops
    -- cop_list[closest_cop_index]=nil
    -- for i,cop in pairs(cop_list) do
        ClearPedTasks(cop)
        TaskGotoEntityAiming(cop, player, 4.0, 4.0)
...
-- surrender.loop=function()
    -- local timer_started
    -- print("handsup timer="..GetGameTimer())
    -- if not HasAnimDictLoaded(surrender.dict) then
        -- RequestAnimDict(surrender.dict)
    -- end
    -- while surrender.in_progress do
        -- if surrender.closest_cop_sdist>16.0 then
            -- local ped=GetPlayerPed(-1)
            -- surrender.loop_cops(ped)
            -- if surrender.closest_cop then
                -- ClearPedTasks(surrender.closest_cop)
                -- TaskGotoEntityAiming(surrender.closest_cop, ped, 3.0, 3.0)
                -- SetPedKeepTask(surrender.closest_cop, true)
            -- end
            -- surrender.player_action=surrender.handsup
            -- Wait(0)
        -- else
            -- timer_started=GetGameTimer()
            -- print("cuffed timer="..timer_started)
            -- surrender.player_action=function() Wait(100) end
            -- while surrender.in_progress and (timer_started+2000)>GetGameTimer() do
                -- local ped=GetPlayerPed(-1)
                -- surrender.loop_cops(ped)
                -- if surrender.closest_cop then
                    -- ClearPedTasks(surrender.closest_cop)
                    ...
                    -- SetPedKeepTask(surrender.closest_cop, true)
                -- end
                -- --if closest_cop_sdist>36.0 then
                -- --    surrender.in_progress=false
               -- --     return
               -- --end
                -- surrender.disablecontrols()
                -- Wait(0)
            -- end
            -- timer_started=GetGameTimer()
            -- TaskPlayAnim(GetPlayerPed(-1), surrender.dict_walking, surrender.anim_walking, 8.0, -8, -1, 49, 0, 0, 0, 0)
    -- SetCurrentPedWeapon(GetPlayerPed(-1), GetHashKey("WEAPON_UNARMED"), true)
            -- print("getincar timer="..timer_started)
            -- surrender.player_action=surrender.getincar
            -- while surrender.in_progress and (timer_started+20000)>GetGameTimer() and not IsPedInVehicle(GetPlayerPed(-1),surrender.veh,false) do
                -- local ped=GetPlayerPed(-1)
                -- surrender.loop_cops(ped)
                -- if surrender.closest_cop then
                    -- ClearPedTasks(surrender.closest_cop)
            ...
            -- end
            -- timer_started=GetGameTimer()
            -- print("to police station timer="..timer_started)
            -- ClearPedTasks(surrender.closest_cop)
            -- SetPedRelationshipGroupHash(GetPlayerPed(-1),GetHashKey("COP"))
            -- --SetPedAsCop(GetPlayerPed(-1),true)
            -- SetPedAsGroupMember(surrender.closest_cop,GetPedGroupIndex(GetPlayerPed(-1)))
            -- Wait(0)
            -- while surrender.in_progress and (timer_started+60000)>GetGameTimer() and IsPedInVehicle(GetPlayerPed(-1),surrender.veh,false) do
                -- surrender.drivetopolicestation()
            -- end
            -- print("end timer="..GetGameTimer())
            -- surrender.in_progress=false
        -- end
    -- end
end
--Citizen.CreateThread(surrender.loop) --debug

Citizen.CreateThread(function()
    surrender.dict,surrender.anim='random@arrests','kneeling_arrest_idle'
    RequestAnimDict(surrender.dict)
    while not HasAnimDictLoaded(surrender.dict) do Wait(10) end
    while true do
        Wait(0)
        local ped=GetPlayerPed(-1)
        if (not IsPedInAnyVehicle(ped)) and IsControlPressed(0,74) then
            if player_is_cop then
                ...
                for i=0,31 do
                    ...
                    if suspect~=cop then
                            print("3 check done")
                    if IsEntityPlayingAnim(suspect,surrender.dict,surrender.anim,3) then
                            print("check done")
                ...
                ClearPedTasks(GetPlayerPed(-1))
                surrender.in_progress=true
                surrender.cops_around=0
                surrender.closest_cop=nil
                surrender.closest_cop_sdist=1000000.0
                --surrender.veh=nil
                --surrender.player_action=surrender.handsup
                --surrender.wanted_level=GetPlayerWantedLevel(PlayerId())
                TaskPlayAnim(GetPlayerPed(-1), surrender.dict, surrender.anim, 2.0, -8.0, -1, 0, 0, 0, 0, 0)
                Wait(500)
                Citizen.CreateThread(surrender.loop_cops)
                Wait(1000)
                while surrender.in_progress do
                    surrender.handsup()
                    --surrender.player_action()
                end
                Wait(1000)
